...
// QUESTA È LA FUNZIONE MODIFICATA: SOLO TRADUZIONE!
function mergeSubtitles(mainSubs, transSubs, mergeThresholdMs = 500) {
    ...
    for (const mainSub of mainSubs) {
        ...
        if (bestMatchIndex !== -1) {
            ...
            mergedSubs.push({
                ...mainSub,
                text: '' // Nessuna traduzione trovata
            });
        }
    }
...
// (Le altre funzioni di parsing, buildSearchUrl, e la logica principale del tuo addon vanno qui, come nel file originale)

function buildSearchUrl(params) {
    // Per esempio:
    // https://rest.opensubtitles.org/search/episode-1/imdbid-1234567/season-2/sublanguageid-eng
    let url = `${OPENSUBS_API_URL}/search`;
    if (params.episode) {
        url += `/episode-${params.episode}`;
    }
    if (params.imdbid) {
        url += `/imdbid-${params.imdbid}`;
    }
    if (params.season) {
        url += `/season-${params.season}`;
    }
    if (params.sublanguageid) {
        url += `/sublanguageid-${params.sublanguageid}`;
    }
    return url;
}

// ... (Qui va la logica principale del tuo addon, come la gestione delle richieste Stremio, parsing SRT, ecc.)
